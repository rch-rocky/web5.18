...
export async function generateStaticParams() {
  const categories = [spe, quechers, ionChromatography, consumables, mycotoxin, specialized, chromatography]
    ...
    if (category.subcategories) {
      return category.subcategories.flatMap(subcategory => 
        subcategory.products?.filter(product => {
          // 确保产品ID在对应的分类中有效
          if (category.id === 'chromatography') {
            return chromatography.subcategories.some(sc => 
              sc.products.some(p => p.id === product.id)
            )
          }
          return true
        }).map(product => ({
...
export default async function Page({ params }: { params: { id: string; productId: string } }) {
  ...
  id === 'chromatography' ? chromatography :
  spe
